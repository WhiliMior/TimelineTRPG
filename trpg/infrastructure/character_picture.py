...
class CharacterPictureGenerator:
    ...
    BASIC_INFO_CONFIGS = [
        ...
        # 右侧（外貌、资产、年龄、体型）
        AttributeConfig(
            "外貌", "", "Appearence.png", 426, 287, 476, 288, FONT_SIZE_SMALL
        ),
    ...
    DOMAIN_ATTRS = [
        AttributeConfig(
            "医学及生命科学", "医", "Medical.png", 426, 1008, 476, 1008, FONT_SIZE_SMALL
        ),
        AttributeConfig(
            "工程与科技", "工", "Engineer.png", 426, 1074, 476, 1074, FONT_SIZE_SMALL
        ),
        AttributeConfig(
            "军事与生存", "军", "Survival.png", 426, 1140, 476, 1140, FONT_SIZE_SMALL
        ),
        AttributeConfig(
            "文学", "文", "Literature.png", 426, 1205, 476, 1206, FONT_SIZE_SMALL
        ),
        AttributeConfig(
            "视觉及表演艺术", "艺", "Art.png", 426, 1270, 476, 1272, FONT_SIZE_SMALL
        ),
    ]
    ...
    REVISION_CONFIGS = [
        ...
        RevisionConfig(
            "", "Age.png", 76, 1138, 126, 1140, COLOR_AGE_PHYSICAL
        ),  # 物理年龄修正
    ...
    def _load_font(cls, font_file: str, size: int) -> ImageFont.FreeTypeFont:
        ...
        if cache_key not in cls._font_cache:
            ...
            # 3. 都失败则使用默认字体（显示方框）
            if not font_loaded:
                logger.warning("所有字体加载失败，使用PIL默认字体，显示可能异常")
    ...
    def generate_character_picture(
        ...
        # 准备数据
        data = cls._prepare_character_data(
            user_id, character, raw_attributes, final_attributes
        )
    ...
    def _prepare_character_data(
        ...
        # 计算修正值
        data["revision_age_physical"], data["revision_age_mental"] = (
            cls._calculate_age_revision(data["age"], data["adult_age"])
        )
        data["revision_size"] = cls._calculate_size_revision(
            data["size"], data["standard_size"]
        )
        ...
        # 负重
        weight = (
            CharacterReader.get_character_current_weight(user_id, character.get("name"))
            or 0
        )
        full_weight = (
            CharacterReader.get_character_full_weight(user_id, character.get("name"))
            or 0
        )
        data["weight"] = weight
    ...
    def _draw_header(cls, img: PilImage.Image, draw: ImageDraw.Draw, data: dict):
        ...
        # 角色姓名
        cls._draw_icon_text(
            img, draw, icons_dir, "Character.png", (76, 114), (126, 112), data["name"]
        )
    ...
    def _draw_hp_mp(cls, draw: ImageDraw.Draw, data: dict):
        ...
        # HP - 两行格式
        hp_ratio = (
            int((data["hp"] / data["full_hitpoint"]) * 100)
            if data["full_hitpoint"] > 0
            else 0
        )
        # 第一行：HP [百分比]
        ...
        # MP - 两行格式
        mp_ratio = (
            int((data["mp"] / data["full_willpower"]) * 100)
            if data["full_willpower"] > 0
            else 0
        )
        # 第一行：MP [百分比]
    ...
    def _draw_attributes(cls, img: PilImage.Image, draw: ImageDraw.Draw, data: dict):
        ...
        for config in cls.PHYSICAL_ATTRS:
            ...
            cls._draw_attribute_with_icon(
                img, draw, icons_dir, config, value, buff_amount
            )
    ...
    def _draw_attribute_with_icon(
        ...
        # 绘制文字和buff
        cls._draw_attribute_with_buff(
            draw,
            config.short_name,
            value,
            buff_amount,
            (config.x_text, config.y_text),
            config.font_size,
        )
    ...
    def _draw_revisions(cls, img: PilImage.Image, draw: ImageDraw.Draw, data: dict):
        ...
        cls._draw_icon_text(
            img, draw, icons_dir, "Weight.png", (76, 1008), (126, 1008), text
        )
        ...
        text = f"修正 [{revision}%]"
        cls._draw_icon_text(
            img, draw, icons_dir, "Weight.png", (76, 1074), (126, 1074), text
        )
        ...
        cls._draw_icon_text(
            ...
            text_color=cls.COLOR_AGE_PHYSICAL,
        )
        ...
        cls._draw_icon_text(
            ...
            text,
            font_size=cls.FONT_SIZE_SMALL,
            text_color=cls.COLOR_AGE_MENTAL,
        )
    ...
    def _draw_attribute_with_buff(
        ...
        if buff_amount != 0:
            # 有buff，显示差异（带颜色）
            buff_str = (
                f"(+{int(buff_amount)})" if buff_amount > 0 else f"({int(buff_amount)})"
            )
            ...
            draw.text(
                (buff_x, coord[1] + (font_size - cls.FONT_SIZE_BUFF) // 2 + 2),
                buff_str,
                buff_color,
                font=buff_font,
            )
