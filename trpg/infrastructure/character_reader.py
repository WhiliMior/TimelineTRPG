...
class CharacterReader:
    ...
    @staticmethod
    def get_active_character(user_id: str) -> dict | None:
        """
        获取当前激活的角色
    ...
    @staticmethod
    def get_character_by_name(user_id: str, character_name: str) -> dict | None:
        """
        根据角色名获取角色
    ...
    @staticmethod
    def get_character_raw_attributes(
        user_id: str, character_name: str = None
    ) -> dict | None:
        """
        ...
        raw_attributes = CharacterReader.create_raw_attributes(
            basic_attributes, user_id, character_name
        )

        return raw_attributes

    @staticmethod
    def get_character_final_attributes(
        user_id: str, character_name: str = None
    ) -> dict | None:
        """
        ...
        """
        # 先获取原始属性
        raw_attributes = CharacterReader.get_character_raw_attributes(
        ...
        final_attributes = CharacterReader.apply_buffs_to_attributes(
            raw_attributes, buffs
        )
    ...
    @staticmethod
    def get_active_character_attributes(
        user_id: str, include_buffs: bool = True
    ) -> dict | None:
        """
        获取当前激活角色的属性（便捷方法）
    ...
    def get_attribute_value(
        user_id: str, attribute_name: str, include_buffs: bool = True
    ) -> float | None:
        """
        获取指定属性的值
    ...
    @staticmethod
    def resolve_attribute_name(attribute: str) -> str | None:
        """
        解析属性名称，使用别名映射
    ...
    def apply_buffs_to_attributes(raw_attributes: dict, buffs: list) -> dict:
        ...
        for attr_name, attr_value in raw_attributes.items():
            if CharacterReader.is_number(attr_value):
                # 获取影响这个属性的所有buff
                buffs_for_attr = CharacterReader._get_buffs_for_attribute(
                    attr_name, buffs
                )
                # 按照buff计算逻辑对属性值进行计算
                final_value = CharacterReader._calculate_buff_impact(
                    attr_value, buffs_for_attr
                )
    ...
    def _get_buffs_for_attribute(attribute_name: str, buffs: list) -> list:
        ...
        for buff in buffs:
            ...
            if scope == attribute_name or scope == "所有" or scope == "全部":
                ...
                attribute_name in mental_attributes
                or attribute_name in domain_attributes
            ):
                relevant_buffs.append(buff)
    ...
    @staticmethod
    def _calculate_buff_impact(
        attribute_value: float, buffs_for_attribute: list
    ) -> float:
        """
        按照buff计算逻辑对属性值进行计算：
        ...
        # 应用计算公式
        result = (
            ((attribute_value + direct_add) * direct_multiply) + final_add
        ) * final_multiply
    ...
    @staticmethod
    def create_raw_attributes(
        character: dict, user_id: str = None, character_name: str = None
    ) -> dict:
        """
        ...
        if user_id and character_name:
            current_weight = CharacterReader.get_character_current_weight(
                user_id, character_name
            )
        ...
        revision_weight = CharacterReader._calculate_revision_weight(
            weight, full_weight
        )
        ...
        # 替换领域属性
        add_attribute(
            "医学及生命科学", revised_mental * basic_values["医学及生命科学"] / 100
        )
        ...
        add_attribute(
            "视觉及表演艺术", revised_mental * basic_values["视觉及表演艺术"] / 100
        )
    ...
    @staticmethod
    def _calculate_negotiation_buff(
        attribute_dict: dict, character: dict, physical: float, mental: float
    ) -> float:
        """
        计算交涉加成属性
        ...
        # 从角色的data字段获取外貌值
        appearance = CharacterReader._read_basic_attribute(
            character.get("data", {}), "外貌"
        )
        ...
        negotiation_buff = (
            (literature + visual_and_performing_art + intelligence)
    ...
    def get_character_full_weight(
        ...
        # 计算 full_weight
        full_weight = CharacterReader._calculate_full_weight_from_character(
            basic_attributes
        )
        return full_weight
    ...
    @staticmethod
    def get_character_current_weight(
        user_id: str, character_name: str = None
    ) -> float | None:
        """
        获取角色当前负重（物品 + 武器）
