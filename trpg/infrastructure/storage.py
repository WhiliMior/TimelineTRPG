...
class StorageBackend:
    ...
    def _get_entity_dir(cls, storage_type: StorageType, entity_id: str) -> Path:
        ...
        # 替换 Windows 不允许的路径字符
        safe_entity_id = (
            entity_id.replace(":", "_").replace("/", "_").replace("\\", "_")
        )
    ...
    @classmethod
    def _get_file_path(
        cls, storage_type: StorageType, entity_id: str, filename: str = None
    ) -> Path:
        """
        获取文件路径
        ...
        if filename is None:
            ...
            filename = filename_map.get(
                storage_type, f"{storage_type.value.lower()}.json"
            )
    ...
    @classmethod
    def save(
        cls, storage_type: StorageType, entity_id: str, data: Any, filename: str = None
    ) -> bool:
        """
        保存数据
    ...
    @classmethod
    def load_global(
        cls, storage_type: StorageType, filename: str, default: Any = None
    ) -> Any:
        """
        加载全局数据文件
    ...
    def load_battle(cls, conversation_id: str) -> Dict:
        ...
        Returns:
            战斗数据
        """
        default = {"active_battle_id": None, "player": {}, "battle_list": {}}
    ...
    def load_negotiation(
        ...
        Returns:
            谈判数据
        """
        data = cls.load_global(
            StorageType.EXAMINATION,
            "negotiation.json",
    ...
    def load_target(
        ...
        """
        data = cls.load_global(
            StorageType.EXAMINATION, "target.json", default={"group": {}, "private": {}}
        )
    ...
    @classmethod
    def _get_battle_timeline_dir(
        cls, session_type: SessionType, conversation_id: str
    ) -> Path:
        """
        获取战斗时间轴目录
        ...
        """
        base_dir = (
            cls._get_plugin_data_dir()
            / StorageType.BATTLE_TIMELINE.value
            / session_type.value
        )
        # 替换 Windows 不允许的路径字符
        safe_id = (
            str(conversation_id).replace(":", "_").replace("/", "_").replace("\\", "_")
        )
    ...
    @classmethod
    def _get_battle_timeline_file_path(
        cls, session_type: SessionType, conversation_id: str
    ) -> Path:
        """
        获取战斗时间轴文件路径
    ...
    @classmethod
    def _get_inventory_dir(cls, is_group: bool, conversation_id: str) -> Path:
        """
        获取物品重量存储目录
    ...
    def load_inventory_weights(
        ...
        Returns:
            物品重量数据字典 {物品名: 重量}
        """
        file_path = cls._get_inventory_dir(is_group, conversation_id) / "weights.json"
