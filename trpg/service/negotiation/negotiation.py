...
class NegotiationModule:
    ...
    async def neg(self, ctx: CommandContext) -> bool:
        ...
        if len(arg_list) == 1:
            ...
            try:
                ...
                self._set_negotiation_target(conversation_id, target_level, target_intelligence, session_type)
    ...
    def _get_negotiation_target(self, conversation_id: str, session_type: str = "private") -> Optional[Dict]:
        return StorageBackend.load_negotiation(conversation_id, session_type)
    
    def _set_negotiation_target(self, conversation_id: str, level: int, intelligence: int, session_type: str = "private"):
        data = {
            "level": level,
    ...
    async def _perform_negotiation(self, ctx: CommandContext, conversation_id: str, rp_grade: float) -> str:
        ...
        # 获取最终属性（包含buff）
        final_attributes = CharacterReader.get_character_final_attributes(user_id, character_name)
        ...
        # 计算成功率
        success_rate = self._calculate_success_rate(rp_grade, negotiation_value, target_intelligence, level, target_level)
    ...
    def _calculate_success_rate(self, rp_grade: float, negotiation_value: float, target_intelligence: float, 
                                level: float, target_level: float) -> float:
        """
        计算交涉成功率
        ...
        success_rate = ((rp_grade + (negotiation_value / level)) / target_intelligence * 25) * (math.log(level / target_level, math.e) + 1)
