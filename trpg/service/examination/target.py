...
class TargetModule:
    ...
    async def tar(self, ctx: CommandContext) -> bool:
        ...
        if not ctx.args:
            ...
            if "d" in cmd1.lower():
                try:
                    ...
                    if number_list:
                        ...
                        result = await self._set_random_target(
                            conversation_id, level, session_type
                        )
                ...
                try:
                    ...
                    result = await self._set_target_value(
                        conversation_id, target_value, session_type
                    )
                    if result["status"] == "success":
                        response = self.reply.render(
                            "tar_set", value=result["target_value"]
                        )
                    else:
                        response = self.reply.render("invalid_target_value")
                except ValueError:
            ...
            try:
                ...
                result = await self._calculate_target(
                    conversation_id, float_cmd1, float_cmd2, session_type
                )
                if result["status"] == "success":
                    response = self.reply.render(
                        "tar_set", value=result["target_value"]
    ...
    async def _set_target(
        self, conversation_id: str, target_value: float, session_type: str = "private"
    ):
        """设置目标值"""
        data = {"target_value": int(target_value)}
        StorageBackend.save_target(conversation_id, data, session_type)

    async def _get_current_target(
        self, conversation_id: str, session_type: str = "private"
    ):
        """获取当前目标值（返回元组）"""
        target_value = await self._get_target(conversation_id, session_type)
    ...
    async def _set_random_target(
        self, conversation_id: str, level: int, session_type: str = "private"
    ) -> dict[str, Any]:
        """设置随机目标值"""
        try:
            level = int(level)
