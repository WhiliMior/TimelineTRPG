...
class ResourceModifierModule:
    ...
    def _normalize_range(self, range_input: str) -> str | None:
        """标准化范围输入"""
    ...
    def _parse_value(self, value_str: str) -> dict | None:
        """
    ...
    def _format_modifier_display(
        ...
        if value_type == "percentage":
            ...
            try:
                # 计算换算后的百分比
                calculated = self._calculate_modifier_value(
                    value_data, character_data, range_val
                )
    ...
    async def _add_modifier(
        ...
        if await self._save_modifiers(user_id, modifiers):
            ...
            value_display = self._format_modifier_display(
                value, character_data, normalized_range
            )
            return self.reply.render(
                "modifier_added", source=source, range=range_input, value=value_display
            )
        else:
    ...
    async def _list_modifiers(
        self, user_id: str, range_filter: str | None = None
    ) -> str:
        """列出资源修饰"""
        character = await self._get_active_character(user_id)
        if not character:
        ...
        if range_filter:
            ...
            if normalized_filter:
                modifiers = [
                    mod for mod in modifiers if mod.get("range") == normalized_filter
                ]
            else:
                return self.reply.render("no_modifiers")
        ...
        for i, modifier in enumerate(modifiers):
            ...
            # 使用格式化方法显示数值
            value_display = self._format_modifier_display(
                value, character_data, range_val
            )
            ...
            lines.append(
                f"[{i + 1}] {source} {range_val} {value_display}{type_display}{duration_display}"
            )

        return "\n".join(lines)

    def _schedule_modifier_event(
        self,
        conversation_id: str,
        user_id: str,
        modifier_id: str,
        ...
        schedule_event(
            ...
            callback_path="trpg.service.resource.modifier.remove_expired_modifier",
            callback_args={"user_id": user_id, "modifier_id": modifier_id},
