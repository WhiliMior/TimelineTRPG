...
class ResourceRecordModule:
    ...
    def _parse_value_with_percentage(self, value_str: str, max_value: Optional[float]) -> Optional[float]:
        ...
        if value_str.endswith('%'):
            ...
            try:
                ...
                return game_config.round_value(max_value * percentage / 100.0, "resource")
            except ValueError:
                return None
        else:
            try:
    ...
    async def _apply_shield_protection(self, user_id: str, resources: Dict, resource_type: str, value_change: float) -> Tuple[float, str]:
        """
        ...
        for shield in sorted_shields:
            ...
            shield['value'] = game_config.round_value(shield['value'] - shield_absorb, "resource")
            ...
            if coverage_name in ['all', '所有']:
                shield_info.append(f"全类型护盾: {game_config.format_value(shield_absorb, 'resource')} (调整: {game_config.format_value(shield_before, 'resource')}→{game_config.format_value(shield_after, 'resource')})")
            else:
                shield_info.append(f"{coverage_name}护盾: {game_config.format_value(shield_absorb, 'resource')} (调整: {game_config.format_value(shield_before, 'resource')}→{game_config.format_value(shield_after, 'resource')})")
        
        # 移除值为0的护盾
        resources['shields'] = [shield for shield in sorted_shields if shield.get('value', 0) > 0]
        ...
        if shield_info:
            # 每种护盾分开显示，换行
            shield_msg = "\n".join(shield_info)
            return actual_change, f"\n承伤: {game_config.format_value(damage_to_absorb, 'resource')}→{game_config.format_value(damage_after_shield, 'resource')}\n护盾保护:\n{shield_msg}"
        else:
            return value_change, ""
    
    async def _apply_modifiers(self, user_id: str, resource_type: str, value_change: float, 
                                change_type: Optional[str] = None) -> Tuple[float, str]:
        """
        应用资源修饰到资源变化值上
        ...
        for mod in percentage_modifiers:
            ...
            mod_value = self._calculate_modifier_value_from_raw(mod_value_data, character_data, mod.get('range', ''))
            ...
            if mod_range.startswith('+'):
                ...
                calculation_steps.append(f"{mod_source}: {before_value:.1f} + {increase:.1f}({display_val}) = {result_value:.1f}")
            else:
                # 减伤：减少百分比
                before_value = result_value
                ...
                result_value = result_value * (1 - mod_value)
                modifier_details.append(f"{mod_source}: -{display_val}")
                calculation_steps.append(f"{mod_source}: {before_value:.1f} - {reduction:.1f}({display_val}) = {result_value:.1f}")
        ...
        for mod in fixed_modifiers:
            ...
            if mod_range.startswith('+'):
                ...
                calculation_steps.append(f"{mod_source}: {before_value:.1f} + {mod_value:.1f} = {result_value:.1f}")
            else:
                # 减伤：减少固定值
                before_value = result_value
                result_value = result_value - mod_value
    ...
    def _calculate_modifier_value_from_raw(self, value_data: Dict, character_data: Dict, range_val: str) -> float:
        """
        根据保存的原始值计算实际的修饰数值
        """
    ...
    async def _handle_resource(self, user_id: str, resource_type: str, args: List[str]) -> str:
        ...
        if resource_type in ['hp', 'mp']:
            value_change, modifier_info = await self._apply_modifiers(user_id, resource_type, value_change, change_type)
        else:
            modifier_info = ""
        ...
        if resource_type in ['hp', 'mp'] and value_change < 0:
            value_change, shield_info = await self._apply_shield_protection(user_id, resources, resource_type, value_change)
        else:
            shield_info = ""
    ...
    async def _handle_shield(self, user_id: str, conversation_id: str, args: List[str]) -> str:
        ...
        shields = resources.get('shields', [])
        shields_before = sum(
            s.get('value', 0) for s in shields 
    ...
    async def _get_resource_status(self, user_id: str) -> str:
        ...
        if max_hp is not None:
            lines.append(f"体力：{game_config.format_value(current_hp, 'resource')}/{game_config.format_value(max_hp, 'resource')}")
        else:
            lines.append(f"体力：{game_config.format_value(current_hp, 'resource')}/未知")
        
        # MP
        if max_mp is not None:
            lines.append(f"意志：{game_config.format_value(current_mp, 'resource')}/{game_config.format_value(max_mp, 'resource')}")
        else:
            lines.append(f"意志：{game_config.format_value(current_mp, 'resource')}/未知")
        ...
        if shields:
            ...
            if hp_shield > 0:
                lines.append(f"体力类型： {game_config.format_value(hp_shield, 'resource')}")
            if mp_shield > 0:
                lines.append(f"意志类型：{game_config.format_value(mp_shield, 'resource')}")
            if all_shield > 0:
                lines.append(f"全类型：{game_config.format_value(all_shield, 'resource')}")
        else:
            lines.append("护盾：无")
