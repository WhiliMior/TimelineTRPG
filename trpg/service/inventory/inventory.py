...
class InventoryModule:
    ...
    async def i(self, ctx: CommandContext) -> bool:
        ...
        # 尝试匹配: 物品名 +/-数量 单个重量
        item_match = re.match(r'^(.+?)\s*([+-])(\d+\.?\d*)\s+(\d+\.?\d*)$', raw_args_for_parse)
        if item_match:
            ...
            result = await self._update_item(user_id, item_name, int(quantity), weight)
            ctx.send(result)
            return True
        ...
        if item_match:
            ...
            result = await self._update_item(user_id, item_name, int(quantity), None)
            ctx.send(result)
            return True
    ...
    async def _calculate_total_weight(self, user_id: str) -> float:
        """计算总负重（物品+武器）- 使用统一接口"""
        from ...infrastructure.character_reader import CharacterReader
        ...
        current_weight = CharacterReader.get_character_current_weight(user_id, character.get('name'))
        return current_weight if current_weight is not None else 0.0
    ...
    async def _get_inventory_status(self, user_id: str) -> str:
        ...
        # 使用 CharacterReader 获取角色的最大负重
        from ...infrastructure.character_reader import CharacterReader
        max_weight = CharacterReader.get_character_full_weight(user_id, character.get('name'))
        
        # 计算总负重
        total_weight = await self._calculate_total_weight(user_id)
    ...
    async def _update_cash(self, user_id: str, amount: float, description: str = "") -> str:
        """更新现金并记录变动"""
        character = await self._get_active_character(user_id)
        if not character:
            return self.reply.render("no_character")
        ...
        if await self._save_inventory_data(user_id, inventory_data):
            if amount >= 0:
                return self.reply.render("cash_added", value=f"+{amount}", old_cash=old_cash, new_cash=new_cash)
            else:
                return self.reply.render("cash_removed", value=str(amount), old_cash=old_cash, new_cash=new_cash)
        else:
            return self.reply.render("save_failed")
    
    async def _update_item(self, user_id: str, item_name: str, quantity: int, weight: float = None) -> str:
        """更新物品（添加或减少）"""
        character = await self._get_active_character(user_id)
        if not character:
            return self.reply.render("no_character")
        ...
        weight_value = game_config.round_value(weight, "weight") if weight is not None else None
        ...
        if await self._save_inventory_data(user_id, inventory_data):
            return self.reply.render("item_added", name=item_name, quantity=abs(quantity))
        else:
            return self.reply.render("save_failed")
