...
class CharacterModule:
    ...
    def _organize_character_data(
        self, attributes: dict[str, any], character_name: str
    ) -> dict[str, any]:
        """
        组织角色数据结构
        """
    ...
    async def tlsetup(self, ctx: CommandContext) -> bool:
        ...
        # 获取角色名称，支持多种名称字段
        character_name = attributes.get(
            "姓名", attributes.get("名称", attributes.get("name", "未命名角色"))
        )
    ...
    async def _get_character_list(self, user_id: str) -> str:
        ...
        for i, character in enumerate(characters):
            ...
            name = character.get(
                "name", self.reply.render("default_character_name", index=str(i + 1))
            )
    ...
    async def _select_character(self, user_id: str, index: int) -> dict[str, str]:
        ...
        if not characters:
            return {"status": "error", "message": self.reply.render("no_character")}

        if index < 0 or index >= len(characters):
            return {"status": "error", "message": self.reply.render("invalid_index")}
        ...
        if await self._save_characters(user_id, characters):
            character_name = characters[index].get(
                "name",
                self.reply.render("default_character_name", index=str(index + 1)),
            )
            return {
                "status": "success",
            ...
            return {"status": "error", "message": self.reply.render("save_failed")}
    ...
    async def _delete_character(self, user_id: str, index: int) -> dict[str, str]:
        ...
        if await self._save_characters(user_id, characters):
            character_name = deleted_character.get(
                "name",
                self.reply.render("default_character_name", index=str(index + 1)),
            )
            return {
    ...
    async def _delete_multiple_characters(
        self, user_id: str, indices: list[int]
    ) -> dict[str, str]:
        """删除多个角色"""
        characters = await self._get_user_characters(user_id)
        ...
        if await self._save_characters(user_id, characters):
            if invalid_indices:
                return {
                    "status": "success",
                    "message": self.reply.render(
                        "character_multi_deleted_partial",
                ...
                return {
                    "status": "success",
                    "message": self.reply.render(
                        "character_multi_deleted", count=len(valid_indices)
                    ),
                }
        else:
            return {"status": "error", "message": self.reply.render("save_failed")}
    ...
    async def get_active_character(self, user_id: str) -> dict | None:
        """获取当前激活角色"""
        characters = await self._get_user_characters(user_id)
        for char in characters:
            if char.get("active", False):
