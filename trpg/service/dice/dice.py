        ...
        if not ctx.args:
            ...
            if len(rolls) == 1:
                ...
                if len(rolls) <= 6:
                    ...
                    # 前6个骰子 + 省略号
                    rolls_display = "+".join(str(r) for r in rolls[:6]) + f"+...(+{len(rolls) - 6})"
            
            if 'd' in args.lower():
                # NdM 格式 或 dN 格式
                response = self.reply.render("roll_result", expression=args, result=total, rolls=rolls_display)
            elif args.startswith('d'):
                # dN 格式
                response = self.reply.render("roll_result", expression=args, result=total, rolls=rolls_display)
            else:
                # 纯数字格式
                response = self.reply.render("rd_result", expression=args, result=total, rolls=rolls_display)
