...
class BuffModule:
    ...
    async def _add_buff(
        ...
        if duration is not None:
            # 构建描述
            action_desc = (
                f"{character_name} Buff{buff_type} {resolved_attribute}{value:+g}"
            )
            callback_message = (
                f"{character_name} Buff{buff_type} {resolved_attribute}{value:+g}"
            )
    ...
    async def _remove_buff(self, user_id: str, attribute: str | None) -> str:
        """删除buff（按属性名）"""
        buffs = await self._get_buffs(user_id)
    ...
    async def _remove_buff_by_index(self, user_id: str, index: int) -> str:
        ...
        return self.reply.render(
            "buff_removed_by_index", index=index, attribute=removed.get("attribute", "")
        )
    ...
    async def _list_buffs(self, user_id: str, attribute: str | None = None) -> str:
        """显示buff列表"""
        buffs = await self._get_buffs(user_id)
