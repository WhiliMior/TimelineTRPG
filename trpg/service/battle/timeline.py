...
class TimelineModule:
    ...
    def __init__(self):
        ...
        self._timeline_core = TimelineCore()
    ...
    async def tl(self, ctx: CommandContext) -> bool:
        ...
        if main_command == "new":
            ...
            result = self._create_timeline(
                storage_key, user_id, is_group, remaining_args
            )
            ctx.send(result)
    ...
    def _list_timelines(self, storage_key: str, user_id: str, is_group: bool) -> str:
        ...
        # 按时间线ID排序
        sorted_ids = sorted(
            battle_list.keys(), key=lambda x: int(x) if x.isdigit() else 0
        )
    ...
    def _select_timeline(
        self, storage_key: str, user_id: str, is_group: bool, index: int
    ) -> str:
        data = self._get_battle_data(storage_key, is_group)
        battle_list = data.get("battle_list", {})
        ...
        return self.reply.render(
            "timeline_selected", name=selected_battle.get("name", "未命名")
        )

    def _delete_timeline(
        self, storage_key: str, user_id: str, is_group: bool, arg: str
    ) -> str:
        data = self._get_battle_data(storage_key, is_group)
        battle_list = data.get("battle_list", {})
        ...
        if len(valid_indices) == 1:
            ...
            if invalid_indices:
                ...
                return self.reply.render(
                    "timeline_multi_deleted", count=len(valid_indices)
                )
    ...
    def _set_accuracy(self, arg: str) -> str:
        ...
        try:
            ...
            if game_config.set_precision("time", precision):
                min_unit = game_config.get_min_time_unit()
                return self.reply.render(
                    "timeline_accuracy_set", precision=precision, min_unit=min_unit
                )
            else:
    ...
    def _show_timeline(
        ...
        # 实时计算 current_time（通过 self._timeline_core）
    ...
    def _get_time_point(
        self, storage_key: str, user_id: str, is_group: bool, time_point: str
    ) -> str:
        data = self._get_battle_data(storage_key, is_group)
        active_id = data.get("active_battle_id")
        ...
        if ongoing_actions:
            for info in ongoing_actions:
                action = info["action"]
                completed_impact = game_config.round_value(
                    info["completed_impact"], "impact"
                )
                impact_value = game_config.round_value(
                    action.get("impact_value", 0), "impact"
                )
                completion_percentage = game_config.round_value(
