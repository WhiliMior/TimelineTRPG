...
@register(
    "TimelineTRPG", PLUGIN_AUTHOR, "为时间线TRPG规则制作的骰子机器人", PLUGIN_VERSION
)
class TimelineTRPG(Star):
    ...
    def _setup_routes(self):
        ...
        self.router.register(
            "resmod", self._wrap_with_sub_help(resource_modifier_module, "dr")
        )  # 老项目别名
        ...
        self.router.register(
            "rc", self._wrap_with_sub_help(resource_record_module, "rc")
        )
        self.router.register("wp", self._wrap_with_sub_help(weapon_module, "wp"))
        self.router.register(
            "setupWP", self._wrap_with_sub_help(weapon_module, "wp_setup")
        )
        self.router.register("lv", self._wrap_with_sub_help(level_module, "lv"))

        # 注册角色创建指令
        self.router.register(
            "tlsetup", self._wrap_with_sub_help(character_module, "tlsetup")
        )
        self.router.register(
            "TLsetup", self._wrap_with_sub_help(character_module, "tlsetup")
        )  # 大写别名
    ...
    def _collect_help_entries(self):
        ...
        logger.info(
            f"[{PLUGIN_NAME}] 帮助系统已加载: {self.help_registry.list_modules()}"
        )
    ...
    async def trpg_command_handler(
        ...
        ctx = CommandContext(
            ...
            metadata={"message_str": message_str, "raw_event": str(event)},
        )
        ...
        try:
                    ...
                    try:
                        ...
                        if delete_after and os.path.exists(image_path):
                            try:
                                ...
                                logger.debug(
                                    f"[{PLUGIN_NAME}] 已删除临时图片: {image_path}"
                                )
            ...
            yield event.plain_result(
                self.system_reply.render("internal_error", error=str(e))
            )
    ...
    async def initialize(self):
        """插件初始化方法"""
        logger.info(
            f"[{PLUGIN_NAME}] {PLUGIN_VERSION} 初始化完成，adapter 层架构已就绪"
        )
        logger.info(
            f"[{PLUGIN_NAME}] 可用指令: {', '.join(['.' + cmd for cmd in self.router.list_commands()])}"
        )
