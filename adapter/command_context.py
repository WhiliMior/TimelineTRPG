...
class CommandContext:
    ...
    def send(self, text: str, metadata: dict[str, str | int | float | bool | None] | None = None) -> None:
        """
        添加回复消息（不实际发送）
        ...
        # 记录回复内容
        logger.debug(f"[{self.command}] Reply: {text[:100]}{'...' if len(text) > 100 else ''}")
    ...
    def to_dict(self) -> dict[str, str | int | float | bool | None | list[str] | list[ReplyPayload] | dict[str, str | int | float | bool | None]]:
        """转换为字典格式，便于序列化或调试"""
        return {
