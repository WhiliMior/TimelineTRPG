...
class Router:
    ...
    async def dispatch(self, ctx: CommandContext) -> bool:
        ...
        # 记录指令分发信息
        logger.info(f"[Dispatch] command={command}, args={ctx.args}, sender={ctx.sender_name}, group={ctx.group_id}")
        
        if command not in self.routes:
            logger.warning(f"[Dispatch] Command '{command}' not found in routes: {list(self.routes.keys())}")
            ...
            logger.error(f"[Dispatch] Command '{command}' not found and no default handler")
            return False
        ...
        logger.info(f"[Dispatch] Handler result for '{command}': {result}, has_reply={ctx.has_reply()}, reply_count={len(ctx.reply_payloads)}")
        return result
    ...
    def to_dict(self) -> dict[str, str]:
        ...
        for command, handler in self.routes.items():
            result[command] = handler.__name__ if hasattr(handler, '__name__') else str(handler)
        return result